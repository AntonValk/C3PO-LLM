        ...
        for fam in self.model_families:
            for m in self._get_model_list(fam):
                ...
                if self.dataset in ['bigbench_date', 'bigbench_disambiguationQA', 'bigbench_geometric_shapes',
                    ...
                    models[m] = {
                        'responses': [
                            [z.replace('(', '').replace(')', '') if z is not None else z for z in r['decoded_answers']]
...
if __name__ == "__main__":
    ...
    for dataset in ['GSM8K', 'SVAMP']:
        ...
        for thresh in budget_range:
            ...
            cascade_.dqn = copy.deepcopy(cascade.dqn)

            acc, true_dollar_cost, corr_ = cascade_.evaluate()
        ...
        Path(f"logs/{dataset}_{test_dataset}/TREACLE_shift/{family}/boot").mkdir(parents=True, exist_ok=True)
        llm_list = cascade._get_model_list(model_fam[0])
        llm_list = fr'{"".join(llm_list)}'.split("/")
        model_string = "-".join(llm_list)

        with open(f'logs/{dataset}_{test_dataset}/TREACLE_shift/{family}/boot/{model_string}_budget_{num_responses}.json', 'w+') as fp:
            json.dump(res, fp)
