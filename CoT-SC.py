...
def run_CoT_SC(dataset, llm, sampling_budget, N_cal):
    ...
    cost = []
    corr = []
    for i in range(2*N_cal, len(data_["responses"])):
      ...
      'bigbench_temporal_sequences', 'commonsenseQA', 'GSM8K', 'SVAMP', 'math_500']:
        ...
        for llm_family in llm_families:
            ...
            for llm in llm_family:
                ...
                Path(f"logs/{dataset}/CoT_SC/boot").mkdir(parents=True, exist_ok=True)

                llm_name = fr'{llm}'.split("/")[-1]

                with open(f'logs/{dataset}/CoT_SC/boot/{llm_name}.json', 'w+') as fp:
                  json.dump(res, fp)
