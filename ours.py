...
class BruteForceCascadeOptimizer:
    ...
    def get_true_answer(self, q_idx):
        ...
        if self.dataset in ['bigbench_date', 'bigbench_disambiguationQA', 'bigbench_geometric_shapes', 'bigbench_movie_recommendation', 'bigbench_penguins', 'bigbench_ruin_names', 'bigbench_snarks', 'bigbench_temporal_sequences']:
    ...
    for dataset in ['aqua', 'bigbench_causal_judgement', 'bigbench_date', 'bigbench_disambiguationQA',
        ...
        for z in [5]:
            ...
            for i in budget_range:
                ...
                acc, cost, true_dollar_cost, corr = optimizer.evaluate('test')
            ...
            Path(f"logs/{dataset}/Ours/{family}/boot").mkdir(parents=True, exist_ok=True)
            llm_list = optimizer._get_model_list(model_fam[0])
            llm_list = fr'{"".join(llm_list)}'.split("/")
            model_string = "-".join(llm_list)

            with open(f'logs/{dataset}/Ours/{family}/boot/{model_string}_budget_{z}.json', 'w+') as fp:
                json.dump(res, fp)
