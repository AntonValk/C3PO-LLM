...
class BruteForceCascadeOptimizer:
    ...
    def _compute_loss(self, thresholds, indices, split='train'):
        ...
        for i in indices:
            ...
            answer = None
            for j, m in enumerate(self.sorted_models):
            ...
            if answer is None:
                ...
                exit_[i] = len(self.sorted_models)-1
            ...
            if answer != true_ans:
                ...
                corr.append(1)
        return mistakes / len(indices), corr, list(exit_.values())
    ...
    def brute_force_search(self):
        ...
        for combo in tqdm(prod_iter, total=total_combos, desc="Brute‐forcing thresholds"):
            ...
            loss, _, _ = self._compute_loss(thresholds, self.ss_indices)
    ...
    def evaluate(self, split='test'):
        ...
        loss, corr, exits_ = self._compute_loss(self.thresholds, idx, split=split)
        ...
        return 1 - loss, avg_cost, true_avg_cost, corr, exits_
...
if __name__ == "__main__":
    ...
    for dataset in ['math_500']:
        family = "QWEN" #"LLAMA"  # "LLAMA" #"GPT" #"QWEN" #
        ...
        for z in [5]:
            ...
            for i in budget_range:
                ...
                acc, cost, true_dollar_cost, corr, exits_ = optimizer.evaluate('test')
                ...
                res[true_dollar_cost] = [acc, lo, hi, exits_]

            Path(f"logs/{dataset}/Ours_exit/{family}/boot").mkdir(parents=True, exist_ok=True)
            llm_list = optimizer._get_model_list(model_fam[0])
            llm_list = fr'{"".join(llm_list)}'.split("/")
            model_string = "-".join(llm_list)

            with open(f'logs/{dataset}/Ours_exit/{family}/boot/{model_string}_budget_{z}.json', 'w+') as fp:
                json.dump(res, fp)
