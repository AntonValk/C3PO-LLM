    ...
    for dataset in ['math_500']:
        ...
        for thr in np.linspace(0, 1, 5):
            ...
            for run_id in range(n_ens):
                ...
                _, avg_cost, answer_list, ground_truth = cascade.evaluate()
        ...
        Path(f"logs/{dataset}/FrugalGPT/{family}/boot").mkdir(parents=True, exist_ok=True)
        llm_list = cascade._get_model_list(model_fam[0])
        llm_list = fr'{"".join(llm_list)}'.split("/")
        model_string = "-".join(llm_list)
        with open(f'logs/{dataset}/FrugalGPT/{family}/boot/{model_string}_budget_{n_ens}.json', 'w+') as fp:
            json.dump(res, fp)
